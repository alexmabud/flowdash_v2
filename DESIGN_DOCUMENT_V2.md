...
## 📋 Índice
- [🤖 Meta-Instruções para Agentes de IA](#-meta-instruções-para-agentes-de-ia-leia-antes-de-codar)
- [1. Sumário Executivo](#1-🔭-sumário-executivo)
- [2. Arquitetura da Solução](#2-🏛️-arquitetura-da-solução)
- [3. Modelagem de Dados](#3-🗄️-modelagem-de-dados-schema-core)
- [4. Módulos e Serviços](#4-🧠-módulos-e-serviços-backend)
- [5. Integração Frontend](#5-🎨-integração-frontend-nextjs)
- [6. Estratégia de Migração](#6-🔄-estratégia-de-migração-etl)
- [7. Roadmap de Implementação](#7-🗺️-roadmap-de-implementação)
- [8. Mapa do Território](#8-📂-mapa-do-território-árvore-de-arquivos-v2)
...
### 1. Respeite a Camada de Serviço (Service Layer)
...
### 2. Integridade do Ledger (Sagrado)
...
## 3. 🗄️ Modelagem de Dados (Schema Core)
...
| Serviço | Responsabilidade | Origem V1 (Referência) |
| :--- | :--- | :--- |
| **AuthService** | Login (Senha/PIN), Geração e validação de JWT. | `auth/auth.py`, `utils/pin_utils.py` |
| **SalesService** | Registrar venda, aplicar taxas de maquineta, calcular data de liquidação (Workalendar). | `services/vendas.py`, `flowdash_pages/lancamentos/actions_venda.py` |
| **FinanceService** | Registrar saídas simples, transferências entre contas (atomicidade), correções. | `actions_saida.py`, `actions_transferencia.py`, `actions_deposito.py` |
| **LedgerService** | O coração contábil. Baixa de faturas, amortização de empréstimos, cálculo de saldo devedor. | `services/ledger/*.py` (Todos os mixins) |
| **ReportService** | Agregação de dados para DRE e Dashboard (Queries SQL otimizadas). | `dre/dre.py`, `dashboard/dashboard.py` |
| **LockService** | Guardião. Impede alterações em datas já fechadas. | `fechamento/lock_manager.py` |
...
1. **Ler** o SQLite `flowdash_data.db` usando Pandas.
2. **Transformação**:
...
3. **Carga**: Inserir no PostgreSQL via SQLAlchemy Models, respeitando a ordem: `Users` -> `Accounts` -> `Categories` -> `Payables` -> `Transactions`.

## 7. 🗺️ Roadmap de Implementação

- [x] **Setup & Infra**: Criar estrutura de pastas, Docker, configurar DB. (✅ Feito)
...
## 8. 📂 Mapa do Território (Árvore de Arquivos V2)
